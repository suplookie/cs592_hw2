...
class KukaGymEnv(gym.Env):
  ...
  def getExtendedObservation(self):
    ...
    self._observation = [e*10 for e in projectedBlockPos2D]+ (list(blockEulerInGripper))#[0,0,0]
    # --------------------------------------------------------------
  ...
  def step(self, action, **kwargs):
    ...
    if (self._isDiscrete):
      ...
      dx = dv if action % 5 == 0 else (-1)*dv if action % 5 == 1 else 0
      # desired position displacement along y axis 
      dy = dv if action % 5 == 2 else (-1)*dv if action % 5 == 3 else 0
      # desired rotation displacement along yaw axis 
      da = 5*dv if action // 5 == 0 else (-5)*dv if action // 5 == 1 else 0
      # --------------------------------------------------------------
  ...
  def _reward(self):
    ...
    reward -= np.linalg.norm(np.absolute(np.array(self.getExtendedObservation())) - np.array([0.0, 0.0, 2.8, 0, 1.7]))
