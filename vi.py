...
class valueIterAgent:
    ...
    def choose_action(self, state):
        ...
        return np.argmax(policy[self.get_grid_index(state)])
        # -----------------------------------------------------------
        ...
        while True:
            ...
            for i, state in enumerate(self.states):
                ...
                for j, action in enumerate(self.actions):
                    if self.env.isValid(state + action, check_collision=False):
                        index = self.get_grid_index(state + action)
                        if index < len(self.states) or j == 0:
                            nxt_value = max(nxt_value, self.env.reward_fn(state) + self.gamma * values[self.get_grid_index(state + action)])
                nxt_values[i] = nxt_value
                diff = max(diff, abs(values[i] - nxt_values[i]))
    ...
    def learn(self, error=1e-9):
        ...
        # Place your code here
        # -----------------------------------------------------------
        reward = self.env.reward_fn
        for s in range(self.observation_dim):
            max_val = 0
            action_index = -1
            for a in range(self.action_dim):
                new_state = self.states[s] + self.actions[a]
                if self.env.isValid(new_state, check_collision=False):
                    new_state_index = self.get_grid_index(new_state)
                    if new_state_index < len(values):
                        val = values[new_state_index]
                        if action_index == -1 or val > max_val:
                            max_val = val
                            action_index = a
            policy[s][action_index] = 1
