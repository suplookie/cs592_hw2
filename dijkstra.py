...
def dijkstra_planning(start, goal, actions, resolution, grid_limits,
    ...
    while True:
        ...
        for i, action in enumerate(actions):
            pos = cur_node.pos + action
            index = get_grid_index(pos, resolution, grid_limits, grid_dim)
            if index not in openset and index not in closedset:
                if is_valid(pos, grid_limits, obstacle_tree, robot_size):
                    if index == goal_node.idx:
                        goal_node.prev_idx = cur_idx
                    node = Node(pos, index, cur_node.cost + 1, cur_idx)
                    openset[index] = node
    ...
    while prev_idx != start_node.idx:
        node = closedset[prev_idx]
        path.append(node.pos)
        prev_idx = node.prev_idx
